...
class GoogleMapsDirectionLinkTest extends DuskTestCase
{
    use DatabaseMigrations;

    /**
     * TS-27: Mygtuko paspaudimas, atveriantis maršrutą „Google Maps“ platformoje
     */
    public function test_user_can_view_saved_trips_list()
    {
        ...
        $this->browse(function (Browser $browser) use ($trip, $user, $tripTitle) {
            $browser->visit('/login')
                ...
                ->click('.trip-view-link')
                ->waitForText($tripTitle, 5)
                ->clickLink('🗺️ Atidaryti Google Maps 🗺️')
                ->pause(2000)
                ->assertSee('Google Maps')
                ->screenshot('GoogleMapsRouteLink');
        });
    }
}
