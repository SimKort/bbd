...
class TripPdfGenerationTest extends DuskTestCase
{
    use DatabaseMigrations;

    /**
     * TS-26: PDF generavimas ir atsisiuntimas su kelionės duomenimis
     */
    public function test_user_can_view_saved_trips_list()
    {
        ...
        $this->browse(function (Browser $browser) use ($trip, $user, $tripTitle) {
            $browser->visit('/login')
                ...
                ->waitForText($tripTitle, 5)
                ->clickLink('📥 Parsisiųsti kaip PDF 📥')
                ->pause(2000)
                ->screenshot('TripPdfDownload');
        });
    }
}
